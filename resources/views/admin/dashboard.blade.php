<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            {{-- The full picture — what's converting, what's waiting, who queued it,
                 and what's already done — lives in the conversion module now. --}}
            @if ($stats['needs_conversion'] > 0 || $stats['converting'] > 0)
                <a href="{{ route('admin.conversion.index') }}" class="block bg-white dark:bg-pine border border-jd-warning/30 shadow-sm rounded-xl p-4 hover:border-jd-warning/50 transition">
                    <div class="flex items-center justify-between gap-4 flex-wrap">
                        <div>
                            ...
                            <p class="text-xs text-jd-ink-muted dark:text-sage mt-0.5">
                                {{ __('See what\'s converting and by whom, what\'s waiting, and pick what goes next.') }}
                            </p>
                        </div>
                        <span class="font-display font-semibold text-xs px-3 py-2 rounded-lg bg-jd-warning text-white shrink-0">{{ __('Manage conversions') }} &rarr;</span>
                    </div>
                </a>
            @endif
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                ...
                <x-admin.stat-card label="Needs PDF Conversion" :value="$stats['needs_conversion']" accent="jd-warning" :href="route('admin.conversion.index')" />
                <x-admin.stat-card label="Converting Now" :value="$stats['converting']" :href="route('admin.conversion.index')" />
